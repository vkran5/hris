...
import { RecruitementStage } from '@/types/reruitmentStage';

const dataSource: RecruitementStage[] = [
  {
    id: 'A',
    stageCode: 'SCR',
    stageName: 'Screening',
  ...
  },
  {
    id: 'D',
    stageCode: 'OFF-NEG',
    stageName: 'Offer and Negotiation',
    level: 4,
  },
];

const StageTab = () => {
  const [selectedRows, setSelectedRows] = useState<RecruitementStage[]>([]);

  const rows = dataSource.map((element: RecruitementStage) => (
    <Table.Tr
      key={element.id}
    ...
    >
      ...
      <Table.Td>{element.stageCode}</Table.Td>
      <Table.Td>{element.stageName}</Table.Td>
    ...
    <main className="mt-4">
      ...
      >
        <Table.Thead>
          <Table.Tr>
            <Table.Th />
            <Table.Th>Stage Code</Table.Th>
            <Table.Th>Staeg Name</Table.Th>
